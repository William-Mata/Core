...
BEGIN
    DBCC CHECKIDENT (Tela, RESEED, 119 )
    INSERT INTO dbo.Tela (DataHoraCadastro, UsuarioCadastroId, ModuloId, Nome, Status)
    VALUES (SYSUTCDATETIME(), 1, @ModuloComprasId, N'Planejamentos', 1);
END;

IF NOT EXISTS (SELECT 1 FROM dbo.Tela WHERE ModuloId = @ModuloComprasId AND Nome = N'Desejos')
BEGIN
DBCC CHECKIDENT (Tela, RESEED, 120 )
    INSERT INTO dbo.Tela (DataHoraCadastro, UsuarioCadastroId, ModuloId, Nome, Status)
    VALUES (SYSUTCDATETIME(), 1, @ModuloComprasId, N'Desejos', 1);
END;

IF NOT EXISTS (SELECT 1 FROM dbo.Tela WHERE ModuloId = @ModuloComprasId AND Nome = N'Histórico de Produtos')
BEGIN
    DBCC CHECKIDENT (Tela, RESEED, 121)
    INSERT INTO dbo.Tela (DataHoraCadastro, UsuarioCadastroId, ModuloId, Nome, Status)
    VALUES (SYSUTCDATETIME(), 1, @ModuloComprasId, N'Histórico de Produtos', 1);
END;

DECLARE @TelaListasComprasId INT = (SELECT TOP 1 Id FROM dbo.Tela WHERE ModuloId = @ModuloComprasId AND Nome IN (N'Planejamentos', N'ListasCompras'));
DECLARE @TelaDesejosCompraId INT = (SELECT TOP 1 Id FROM dbo.Tela WHERE ModuloId = @ModuloComprasId AND Nome IN (N'Desejos', N'DesejosCompra'));
DECLARE @TelaHistoricoPrecosId INT = (SELECT TOP 1 Id FROM dbo.Tela WHERE ModuloId = @ModuloComprasId AND Nome IN (N'Histórico de Produtos', N'HistoricoPrecos', N'HistoricoProduto'));
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.Funcionalidade WHERE TelaId = @TelaListasComprasId AND Nome = N'Excluir')
        INSERT INTO dbo.Funcionalidade (DataHoraCadastro, UsuarioCadastroId, TelaId, Nome, Status) VALUES (SYSUTCDATETIME(), 1, @TelaListasComprasId, N'Excluir', 1);
END;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.Funcionalidade WHERE TelaId = @TelaDesejosCompraId AND Nome = N'Excluir')
        INSERT INTO dbo.Funcionalidade (DataHoraCadastro, UsuarioCadastroId, TelaId, Nome, Status) VALUES (SYSUTCDATETIME(), 1, @TelaDesejosCompraId, N'Excluir', 1);
END;
...
-- Padroniza nomes, garante vinculacao minima e remove inconsistencias sem apagar dados.
UPDATE f
SET Nome = CASE LOWER(LTRIM(RTRIM(f.Nome)))
               WHEN N'visualizar' THEN N'Visualizar'
               WHEN N'criar' THEN N'Criar'
               WHEN N'editar' THEN N'Editar'
               WHEN N'excluir' THEN N'Excluir'
               ELSE f.Nome
           END
FROM dbo.Funcionalidade f
WHERE LOWER(LTRIM(RTRIM(f.Nome))) IN (N'visualizar', N'criar', N'editar', N'excluir')
  AND f.Nome <> CASE LOWER(LTRIM(RTRIM(f.Nome)))
                    WHEN N'visualizar' THEN N'Visualizar'
                    WHEN N'criar' THEN N'Criar'
                    WHEN N'editar' THEN N'Editar'
                    WHEN N'excluir' THEN N'Excluir'
  ...
  (
      SELECT 1
      FROM dbo.Funcionalidade fx
      WHERE fx.TelaId = f.TelaId
        AND fx.Id <> f.Id
        AND LOWER(LTRIM(RTRIM(fx.Nome))) = LOWER(LTRIM(RTRIM(f.Nome)))
  );

INSERT INTO dbo.Funcionalidade (DataHoraCadastro, UsuarioCadastroId, TelaId, Nome, Status)
SELECT SYSUTCDATETIME(), 1, t.Id, N'Visualizar', 1
FROM dbo.Tela t
WHERE t.Status = 1
  AND NOT EXISTS
  (
      ...
      FROM dbo.Funcionalidade f
      WHERE f.TelaId = t.Id
        AND LOWER(LTRIM(RTRIM(f.Nome))) = N'visualizar'
  );
...
(
    ...
    WHERE LOWER(LTRIM(RTRIM(m.Nome))) IN (N'administracao', N'administração', N'financeiro', N'compras')
      ...
      AND LOWER(LTRIM(RTRIM(t.Nome))) NOT IN (N'historicoproduto', N'historicoprecos', N'historicodeprodutos')
),
Acoes AS
(
    SELECT Nome FROM (VALUES (N'Visualizar'), (N'Criar'), (N'Editar'), (N'Excluir')) v(Nome)
)
INSERT INTO dbo.Funcionalidade (DataHoraCadastro, UsuarioCadastroId, TelaId, Nome, Status)
SELECT SYSUTCDATETIME(), 1, tc.Id, a.Nome, 1
FROM TelasCrud tc
CROSS JOIN Acoes a
WHERE NOT EXISTS
(
    SELECT 1
    FROM dbo.Funcionalidade f
    WHERE f.TelaId = tc.Id
      AND LOWER(LTRIM(RTRIM(f.Nome))) = LOWER(a.Nome)
);
...
JOIN dbo.Modulo m ON m.Id = t.ModuloId
WHERE LOWER(LTRIM(RTRIM(m.Nome))) IN (N'administracao', N'administração', N'financeiro', N'compras')
  AND LOWER(LTRIM(RTRIM(t.Nome))) LIKE N'%documentacao%'
  AND LOWER(LTRIM(RTRIM(f.Nome))) <> N'visualizar'
  AND f.Status = 1;
...
JOIN dbo.Modulo m ON m.Id = t.ModuloId
WHERE LOWER(LTRIM(RTRIM(m.Nome))) IN (N'administracao', N'administração', N'financeiro', N'compras')
  AND LOWER(LTRIM(RTRIM(t.Nome))) IN (N'historicoproduto', N'historicoprecos', N'historicodeprodutos')
  AND LOWER(LTRIM(RTRIM(f.Nome))) <> N'visualizar'
  AND f.Status = 1;
...
WHERE LOWER(LTRIM(RTRIM(m.Nome))) IN (N'administracao', N'administração', N'financeiro', N'compras')
  AND LOWER(LTRIM(RTRIM(t.Nome))) NOT LIKE N'%documentacao%'
  AND LOWER(LTRIM(RTRIM(t.Nome))) NOT IN (N'historicoproduto', N'historicoprecos', N'historicodeprodutos')
  AND LOWER(LTRIM(RTRIM(f.Nome))) NOT IN (N'visualizar', N'criar', N'editar', N'excluir')
  AND f.Status = 1;
GO
