...
public sealed record DespesaDto(long Id, string Descricao, string? Observacao, DateOnly DataLancamento, DateOnly DataVencimento, DateOnly? DataEfetivacao, TipoDespesa TipoDespesa, TipoPagamento TipoPagamento, Recorrencia Recorrencia, int? QuantidadeRecorrencia, bool RecorrenciaFixa, decimal ValorTotal, decimal? ValorTotalRateioAmigos, decimal ValorLiquido, decimal Desconto, decimal Acrescimo, decimal Imposto, decimal Juros, decimal? ValorEfetivacao, string Status, TipoRateioAmigos? TipoRateioAmigos, IReadOnlyCollection<AmigoRateioDto> AmigosRateio, IReadOnlyCollection<DespesaAreaRateioDto> AreasSubAreasRateio, long? ContaBancariaId, long? CartaoId, IReadOnlyCollection<DocumentoDto> Documentos, MeioFinanceiroVinculoDto? Vinculo, IReadOnlyCollection<DespesaLogDto> Logs);
