...
public sealed class ReceitaRepository(AppDbContext dbContext) : IReceitaRepository
{
    ...
    private Task<List<Receita>> ListarCoreAsync(int? usuarioCadastroId, string? filtroId, string? descricao, DateOnly? dataInicio, DateOnly? dataFim, CancellationToken cancellationToken) =>
        dbContext.Receitas
